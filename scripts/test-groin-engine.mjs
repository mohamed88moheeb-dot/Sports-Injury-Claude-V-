...
const stage = (days) => runGroin({ ...base, resisted_adduction_pain: 'mild', ability_to_continue: 'yes', pain_severity_label: 'mild', days_since_injury: days }).plan.current_stage_id;
...
check('check-in withholds today on overnight pain', (() => {
  const out = runGroin({ ...base });
  const cur = out.plan.stages.find((s) => s.is_current);
  const adj = applyGroinCheckIn(cur.sessions[0], { overnight_pain: true }, out);
...
check('chronic groin pain check-in withholds on high in-session pain', (() => {
  ...
  const cur = out.plan.stages.find((s) => s.is_current);
  const adj = applyGroinCheckIn(cur.sessions[0], { pain_during_0_10: 8 }, out);
