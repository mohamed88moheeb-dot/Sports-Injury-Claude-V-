/**
 * app/api/lower-back/route.js
 * ---------------------------------------------------------------------------
 * Single server entry for the lower back engine. Mirrors app/api/it-band/route.js.
 * ---------------------------------------------------------------------------
 */

import { runLowerBack } from '../../../lib/clinical/lowerBackEngine/index.mjs';
import { BETA_META } from '../../../lib/clinical/lowerBackEngine/types.mjs';
...
export async function POST(request) {
  try {
    ...
    const lowerBackInput = body && body.lowerBackInput ? body.lowerBackInput : body;
    const userComment = typeof body?.userComment === 'string' ? body.userComment.slice(0, 500) : '';
    if (!lowerBackInput || typeof lowerBackInput !== 'object') {
      return Response.json({ ok: false, error: 'Missing lowerBackInput' }, { status: 400 });
    }
    const output = runLowerBack(lowerBackInput);
    ...
    if (curIdx !== -1 && stages[curIdx].current_stage_pool) {
      ...
      const composed = await composeSessions({
        ...
        userComment,
        injuryLabel: 'lower back (non-specific low back pain / lumbar radicular pain)',
        betaMeta: BETA_META,
      });
