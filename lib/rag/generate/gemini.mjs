...
export async function callGemini({ system, user, maxTokens = 2400, temperature = 0.2, responseSchema = null }) {
  ...
  try {
    const res = await fetch(url, {
      ...
      // Hard network-level abort: a stalled connection must fail fast into
      // the caller's deterministic fallback, never hang an API route open
      // (observed: occasional indefinite hangs on this endpoint in prod).
