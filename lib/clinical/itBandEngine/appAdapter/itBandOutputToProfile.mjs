...
const ENTITY_TITLES = {
  ...
  possible_lateral_knee_structural_injury: 'Possible structural knee injury — needs assessment',
};
...
export function itBandOutputToProfile(itBandOutput, appAssessment = {}, extras = {}) {
  ...
  const sportParticipation = itBandOutput.sport_participation || deriveEnvelopeFromSharedOutput(itBandOutput);

  const MATCH = { high: 80, moderate: 66, low: 52 };
  const routedScore = Number(itBandOutput.routing?.confidence_score);
  ...
  const matchScore = Number.isFinite(routedScore) && routedScore > 0
    ...
    : (MATCH[itBandOutput.routing?.confidence] ?? 60);
  ...
  return {
    ...
    exactAreaName: (appAssessment.exactArea ? pretty(appAssessment.exactArea).replace(/^\w/, (c) => c.toUpperCase()) : '') || ENTITY_TITLES[entity] || 'IT band',
    gradeName,
    mechanism: describeMechanism(itBandOutput.input),
    ...
    daysSinceInjury: Number.isFinite(Number(itBandOutput.input?.days_since_injury)) ? Number(itBandOutput.input.days_since_injury) : null,
    diagnosisDrivers: diagnosisDrivers(itBandOutput.routing),
