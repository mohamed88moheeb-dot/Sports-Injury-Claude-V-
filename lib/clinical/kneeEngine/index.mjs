...
function buildStagedPlan(entity, currentStageId, band) {
  ...
  const totalWeeksEstimate = (ENTITY_WEEKS_ESTIMATE[entity] || {})[band] || stages.length;
  ...
  // hamstring/quad, so an even split is the honest default (still guarantees
  // >=1 week/stage and an exact-summing total via the shared largest-
  // remainder allocator).
  const stageWeeks = splitStageWeeks(stages, totalWeeksEstimate, {});
