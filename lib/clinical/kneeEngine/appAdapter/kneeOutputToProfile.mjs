...
const BAND_TO_IRR = { lower_functional_impact: 'low', moderate_functional_impact: 'moderate', high_concern_or_review_gated: 'high' };

export function deriveKneeParticipationEnvelope(kneeOutput = {}) {
  ...
  const planStages = kplan?.stages || [];
  const finalStageWeeks = planStages.length ? (planStages[planStages.length - 1].duration_weeks || null) : null;
  const planTotalWeeks = kplan?.total_estimated_weeks || null;

  if (reviewRequired) return deriveSportParticipation({ reviewRequired: true });
  if (KNEE_PARTICIPATION_OVERRIDES[entity]) return KNEE_PARTICIPATION_OVERRIDES[entity];
  return deriveSportParticipation(
    OVERUSE_KNEE[entity]
      ? { kind: 'overuse', irritability: BAND_TO_IRR[band] || 'moderate', band, planTotalWeeks, finalStageWeeks }
      : { kind: 'acute_strain', grade: kneeOutput.input?.laxity_grade ? `grade_${kneeOutput.input.laxity_grade}` : null, band, planTotalWeeks, finalStageWeeks },
  );
}
...
export function kneeOutputToProfile(kneeOutput, appAssessment = {}) {
  ...
  const sportParticipation = kneeOutput.sport_participation || deriveKneeParticipationEnvelope(kneeOutput);
