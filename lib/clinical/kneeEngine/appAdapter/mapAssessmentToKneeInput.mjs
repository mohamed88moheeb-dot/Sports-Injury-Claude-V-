...
// 'yes'/true → true; everything else (incl. 'no') → false.
const asBool = (v) => v === true || v === 'yes';

// Morning-stiffness select → representative minutes for the OA router.
const STIFFNESS_MINUTES = { none: 0, brief: 15, prolonged: 60 };
const stiffnessToMinutes = (v) => {
  if (v == null || v === '') return null;
  if (typeof v === 'number') return v;
  if (v in STIFFNESS_MINUTES) return STIFFNESS_MINUTES[v];
  const n = Number(v);
...
export function mapAssessmentToKneeInput(a = {}) {
  ...
  return {
    ...
    morning_stiffness_minutes: stiffnessToMinutes(kneeAnswers.morning_stiffness_minutes),
    pain_severity_label: painLabel,

    days_since_injury: Number.isFinite(Number(a.daysSince)) ? Number(a.daysSince) : 7,
    surgical_repair_done: asBool(kneeAnswers.surgical_repair_done),
    weeks_since_surgery: kneeAnswers.weeks_since_surgery ?? null,

    febrile_hot_joint: asBool(kneeAnswers.febrile_hot_joint),
    high_energy_trauma: asBool(kneeAnswers.high_energy_trauma),
