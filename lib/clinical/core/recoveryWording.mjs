 ...
 * @returns {{ returnRange: string, rfRecoveryWording: string, recoverySecondaryNote: string|null, planWeeks: number|null }}
 */
export function describeRecoveryTimeline(totalWeeks, prognosis) {
  ...
  const secondaryNote = prognosisDiffers
    ? `Typical recovery for this pattern: ${prognosis}. The programme below is structured as ${planWeeksText}, finishing with a graded return to sport.`
    : null;
  ...
  return {
    ...
    // Numeric total so the UI can plot a REAL time axis (elapsed days vs the
    // programme's length) instead of an unrelated percentage.
