/**
 * lib/clinical/core/checkInCore.mjs
 * ---------------------------------------------------------------------------
 * Injury-agnostic daily check-in adjuster, shared by every rehab engine.
 * Adjusts TODAY's session only — never mutates the plan, jumps stages, or
 * grants clearance. Per-entity overrides (e.g. contusion's swelling-and-ROM
 * gate) are passed in via `entityPolicy`.
 * ---------------------------------------------------------------------------
 */
 ...
 *   overnight_pain, swelling_change, ...entity-specific fields
 * }
 * @param {object} entityPolicy  { withhold_if?: (checkin)=>boolean, withhold_message?: string }
 * @param {object} betaMeta      the calling engine's BETA_META constant
 */
export function adjustToday(todaysSession, checkin = {}, entityPolicy = {}, betaMeta = {}) {
  ...
  return { action, message, adjusted_session: session, session_flag: flag(response, checkin), ...betaMeta, trace };
}
