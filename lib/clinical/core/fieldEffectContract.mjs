...
export const RF_FIELD_EFFECTS = [
  ...
  { field: 'mechanism', label: 'How it happened', category: 'mechanism', effects: [
    ...
    rx('timeline', 'recovery_modifier', false, 'Sprint/kick mechanisms trend to longer RTS.') ] },
  ...
  { field: 'pain_at_rest', label: 'Pain at rest', category: 'symptom', effects: [
    ...
    rx('dose', 'volume_ceiling', false, 'Rest pain = high irritability → cap session volume.'),
  ...
  { field: 'next_day_response', label: 'How it felt next day', category: 'symptom', effects: [
    dx('severity', 'irritability', true),
    rx('dose', 'volume_ceiling', false), rx('progression', 'auto_regulation', false,
      'Worse-next-day = reactive → hold/regress progression.') ] },
  ...
  { field: 'knee_extension_response', label: 'Resisted knee extension', category: 'capacity', effects: [
    ...
    rx('dose', 'loading_tolerance', false) ] },
  { field: 'knee_flexion_response', label: 'Knee-flexion stretch', category: 'capacity', effects: [
    dx('severity', 'severity_band', true), rx('emphasis', 'mobility_block', false) ] },
  ...
  { field: 'resisted_hip_flexion', label: 'Resisted hip flexion', category: 'capacity', effects: [
    dx('differential', 'rf_involvement', true), rx('emphasis', 'tissue_loading_block', false) ] },
  { field: 'ely_test', label: 'Ely test (RF tightness)', category: 'deficit', effects: [
    dx('differential', 'rf_involvement', true),
    rx('emphasis', 'mobility_block', false, 'Positive Ely → weight RF mobility.'),
    rx('selection', 'eccentric_lengthening', false) ] },
  { field: 'single_leg_control', label: 'Single-leg control', category: 'deficit', effects: [
    rx('emphasis', 'motor_control_block', false, 'Poor control → weight motor-control/balance.'),
  ...
  { field: 'sprint_tolerance', label: 'Sprint tolerance', category: 'readiness', effects: [
    rx('dose', 'sport_block_load', false, 'Gates sprint/energy-storage dosing.'),
    rx('selection', 'sport_drills', false), rx('progression', 'rts_gate', false) ] },
  { field: 'kick_tolerance', label: 'Kick tolerance', category: 'sport', effects: [
    rx('selection', 'sport_specific_drills', false), rx('progression', 'rts_gate', false) ] },
  { field: 'movement_confidence', label: 'Movement confidence (psychological)', category: 'readiness', effects: [
    rx('progression', 'rts_psychological_gate', false, 'Low confidence → slow progression, delay clearance.'),
    rx('dose', 'progression_pace', false) ] },

  // ── History / context / demand ──────────────────────────────────────────
  { field: 'previous_injury', label: 'Previous same injury', category: 'history', effects: [
    dx('differential', 'recurrence_prior', true), rx('timeline', 'recovery_modifier', false),
  ...
  { field: 'age_group', label: 'Age band', category: 'context', effects: [
    dx('differential', 'epidemiological_prior', false), rx('timeline', 'recovery_modifier', false),
