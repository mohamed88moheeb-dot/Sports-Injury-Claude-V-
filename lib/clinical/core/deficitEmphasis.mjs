...
export function normalizeDeficitProfile(input = {}) {
  ...
  const pain  = g('pain_severity_label');       // mild | moderate | severe
  const stairs= g('stairs_response');           // none | painful | unable | significant_pain
  const hipfx = g('hip_flexion_response');       // painful | significant_pain | unable | …
  const scar  = input.reported_fibrosis_or_scar_history ?? g('scar_history'); // 'none' | present
  ...
  return {
    ...
    mobility_deficit: oneOf(ely, 'anterior_pain', 'tightness_only') || (Number.isFinite(kflex) && kflex >= 50)
      || (!!scar && !['none', 'no', ''].includes(String(scar))),
    strength_deficit: oneOf(weak, 'marked', 'mild'),
    loading_tolerance:
      oneOf(kext, 'unable') || oneOf(stairs, 'unable') ? 'low'
      : oneOf(kext, 'significant_pain') || oneOf(rhf, 'significant_pain', 'unable')
        || oneOf(hipfx, 'painful', 'significant_pain', 'unable') || oneOf(stairs, 'painful', 'significant_pain') ? 'moderate'
      : 'ok',
    irritability:
      oneOf(nday, 'much_worse') || oneOf(rest, 'moderate', 'constant') || oneOf(pain, 'severe') ? 'high'
      : oneOf(nday, 'worse', 'sore_settled') || oneOf(rest, 'mild') || oneOf(pain, 'moderate') ? 'moderate'
      : 'low',
    sport_readiness:
