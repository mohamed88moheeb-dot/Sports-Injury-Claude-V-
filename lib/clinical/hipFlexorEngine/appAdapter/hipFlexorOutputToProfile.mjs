...
export function hipFlexorOutputToProfile(hipFlexorOutput, appAssessment = {}, extras = {}) {
  ...
  let dayCounter = 1;
  const plan = (hplan?.stages || []).map((stage) => {
  ...
  const matchScore = MATCH[hipFlexorOutput.routing?.confidence] ?? 60;
  ...
  return {
    ...
    exactAreaName: diagnosis.summary || ENTITY_TITLES[entity] || 'Hip flexor',
    gradeName,
