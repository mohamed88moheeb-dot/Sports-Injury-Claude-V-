...
export function gradeVastusStrain(input) {
  ...
  // Recovery estimate by BAMIC grade, with a longer window when the site is
  // intratendinous (site 'c') — tendon-involved strains consistently recover
  // slower than myofascial/musculotendinous ones (McAleer 2022; Lempainen 2022).
  const GRADE_PROGNOSIS = { 1: '2-3 weeks', 2: '4-6 weeks', 3: '8-12+ weeks (may need surgical review)' };
  const TENDON_PROGNOSIS = { 1: '3-4 weeks', 2: '6-9 weeks', 3: '10-16+ weeks (surgical review advised)' };
  const GRADE_WEEKS = { 1: 2.5, 2: 5, 3: 10 };
  const TENDON_WEEKS = { 1: 3.5, 2: 7.5, 3: 13 };
  const prognosis = (site === 'c' ? TENDON_PROGNOSIS : GRADE_PROGNOSIS)[grade] || GRADE_PROGNOSIS[2];
  const totalWeeksEstimate = (site === 'c' ? TENDON_WEEKS : GRADE_WEEKS)[grade] || GRADE_WEEKS[2];

  return {
    grade, site, bamic_label: `${grade}${site}`, band, flags, prognosis, totalWeeksEstimate,
...
// MUSCLE_PHASES share of the total recovery timeline (sums to 1). Foundation
// is brief (settle symptoms); accumulation/transition carry the bulk of
// progressive loading; simulation/resilience are the return-to-sport tail.
const MUSCLE_PHASE_SHARE = { foundation: 0.12, reload: 0.18, accumulation: 0.24, transition: 0.18, simulation: 0.16, resilience: 0.12 };
...
export function runVastusStrain(input) {
  ...
  const stageWeeks = splitStageWeeks(MUSCLE_PHASES, severity.totalWeeksEstimate, MUSCLE_PHASE_SHARE);
  const plan = buildStagedPlan(MUSCLE_PHASES, currentPhase, rankedPool, policy, { stageWeeks, betaMeta: BETA_META });
