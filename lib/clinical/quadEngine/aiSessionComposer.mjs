/**
 * lib/clinical/quadEngine/aiSessionComposer.mjs
 * ---------------------------------------------------------------------------
 * AI-driven session composition for the CURRENT stage only. Everything else
 * (entity routing, safety gates, stage timeline, RTS criteria) stays the
 * deterministic engine untouched — this only decides WHICH exercises (from
 * the stage's already clinically-vetted, tagged pool) go into each of the
 * week's 3 sessions, their dosage (from each exercise's own dosage_by_tier),
 * and how to react to a free-text comment ("no gym equipment", "focus more on
 * strength", "my hamstring also hurts").
 *
 ...
 * falls back to the exact deterministic rotation (buildQuadSession) — the
 * plan NEVER goes empty or unsafe for lack of an AI response.
 * ---------------------------------------------------------------------------
 */

import { callGemini, parseJsonLoose, hasGeminiKey } from '../../rag/generate/gemini.mjs';
import { buildQuadSession, toCard } from './shared/quadSessionCore.mjs';
import { getBookendExercises } from './shared/quadWarmupCooldown.mjs';

const SYSTEM = `You are a sports-rehabilitation session-composition assistant. You will be given:
...
Hard rules:
  ...
  - If the comment mentions a DIFFERENT body part/injury (e.g. "my hamstring also hurts"), do NOT add exercises for it — this pool only covers the current injury. Instead set "out_of_scope_note" explaining the patient should run a separate assessment for that.
 ...
 * @param {object} args.policy            { max_items, monitoring_triggers, stop_rule, card_cautions }
 ...
 */
export async function composeQuadSessions({ pool, stage, policy = {}, userComment = '' }) {
  const deterministic = () => ({
    sessions: [0, 1, 2].map((t) => buildQuadSession(pool, stage, t, policy)),
    mode: 'deterministic',
  });
...
Compose the 3 sessions now.`;
  ...
  const res = await callGemini({ system: SYSTEM, user, responseSchema: buildSchema(maxItems), maxTokens: 2000 });
