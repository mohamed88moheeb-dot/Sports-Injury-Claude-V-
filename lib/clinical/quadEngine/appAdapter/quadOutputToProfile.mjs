 ...
 */
export function quadOutputToProfile(quadOutput, appAssessment = {}) {
  ...
  let dayCounter = 1;
  const plan = (qplan?.stages || []).map((stage) => {
  ...
  const matchScore = MATCH[quadOutput.routing?.confidence] ?? 60;
  ...
  return {
    ...
    exactAreaName: diagnosis.summary || ENTITY_TITLES[entity] || 'Quadriceps',
    gradeName,
