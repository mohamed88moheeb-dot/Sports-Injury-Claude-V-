...
export function resolveConfidence(input, missingCoreFields = []) {
  ...
  // ── Derive RF pattern + clinical flags for the RAG exercise retriever ─────
  // These mirror the logic in rfAssessmentModel.mjs::computeRfDiagnosis so the
  // engine's generatePlan() can personalise exercise selection without importing
  // from the app-adapter layer.
  const { rf_pattern, clinical_flags } = derivePatternAndFlags(input, confidence);
...
// ── Pattern and flag derivation ───────────────────────────────────────────────
/**
 * Derive the injury pattern (A/B/C) and clinical flag set from raw input signals.
 * Used by the RAG exercise retriever to personalise phase exercise selection.
 *
 * Mirrors the pattern logic in rfAssessmentModel.mjs::computeRfDiagnosis — kept
 * in sync manually; the resolver owns this for the engine path.
 */
function derivePatternAndFlags(input, confidencePct) {
  const st = input.rf_self_tests || {};
  const mech = input.mechanism || '';
  const palp = st.palpation || '';
  const ely  = st.ely_test   || '';
  const pop  = st.pop_or_snap;
  const age  = st.age_group  || '';
  const btim = st.bruising_timing || '';
  const atc  = st.ability_to_continue_after || '';

  const clinical_flags = [];

  // Proximal tendon suspect: deep palpation near hip crease OR Ely reproduces ant. pain
  const proximalPalp = palp === 'proximal_deep' || palp === 'gap_defect' || palp === 'deep';
  const elyPositive  = ely === 'anterior_pain' || ely === 'could_not_bend';
  if (proximalPalp || elyPositive) clinical_flags.push('proximal_tendon_suspect');

  // Structural defect: palpable gap
  if (palp === 'gap_defect') clinical_flags.push('structural_defect');

  // Audible pop
  if (pop === 'yes') clinical_flags.push('audible_pop');

  // Functional loss / non-ambulatory
  if (atc === 'assisted_off' || atc === 'could_not_bear_weight') {
    clinical_flags.push('non_ambulatory');
  } else if (atc === 'stopped_immediately') {
    clinical_flags.push('functional_loss');
  }

  // Early bruising
  if (btim === 'within_2h_significant' || btim === 'within_2h_some') {
    clinical_flags.push('immediate_bruising');
  } else if (btim === 'delayed') {
    clinical_flags.push('early_bruising');
  }

  // Intratendinous / central tendon suspect: proximal + Ely positive (worst stretch load)
  if (proximalPalp && elyPositive) clinical_flags.push('intratendinous_suspect');

  // Avulsion concern: under-20 kicker with proximal signals
  if (age === 'under_20' && mech === 'kicking' && clinical_flags.includes('proximal_tendon_suspect')) {
    clinical_flags.push('avulsion_concern');
  }

  // High confidence + proximal pattern
  if (clinical_flags.includes('proximal_tendon_suspect') && confidencePct >= 55) {
    clinical_flags.push('proximal_tendon_suspect'); // already there — no duplicate
  }

  // Structural tear
  if (palp === 'gap_defect' && pop === 'yes') {
    clinical_flags.push('structural_tear_pattern');
  }

  // ── Pattern classification ─────────────────────────────────────────────────
  const isProximalKicking = mech === 'kicking' && (
    clinical_flags.includes('proximal_tendon_suspect') ||
    clinical_flags.includes('structural_defect') ||
    clinical_flags.includes('avulsion_concern')
  );
  const isMTJSprint = (mech === 'sprinting' || mech === 'acceleration') && (
    clinical_flags.includes('intratendinous_suspect') ||
    palp === 'deep' || palp === 'proximal_deep'
  );

  let rf_pattern;
  if (isProximalKicking) {
    rf_pattern = { type: 'A', label: 'Proximal / Kicking type', detail: 'Proximal tendon or central tendon involvement. Highest re-injury risk.' };
  } else if (isMTJSprint) {
    rf_pattern = { type: 'B', label: 'Musculotendinous / Sprint type', detail: 'MTJ or intramuscular involvement. Responds well to progressive eccentric loading.' };
  } else {
    rf_pattern = { type: 'C', label: 'Myofascial / Peripheral type', detail: 'Peripheral myofascial injury. Best prognosis. Early mobilisation appropriate.' };
  }

  // Deduplicate flags
  return { rf_pattern, clinical_flags: [...new Set(clinical_flags)] };
}
