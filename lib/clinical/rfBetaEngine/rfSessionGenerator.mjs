...
import { getWarmupContent, getCooldownContent } from './rfWarmupCooldown.mjs';

/**
 * Build a warm-up / cooldown "bookend" card in the exact session-card shape so
 * it renders identically to every other card. Phase-appropriate content comes
 * from rfWarmupCooldown; these frame every session (warm-up first, cooldown last).
 */
function makeBookendCard(block, content, phaseId, kind) {
  return {
    block,
    preview_only: false,
    exercise_id: `RF-${kind.toUpperCase()}-${String(phaseId).toUpperCase()}`,
    exercise_name: content.exercise_name,
    purpose: content.purpose,
    sets: content.dosage.sets,
    reps_or_hold: content.dosage.reps_or_hold,
    rest: content.dosage.rest,
    intensity: content.dosage.intensity,
    tempo: content.dosage.tempo,
    frequency: 'every session',
    equipment: content.equipment || [],
    instructions: content.instructions || [],
    coaching_cues: content.coaching_cues || [],
    common_mistakes: content.common_mistakes || [],
    alternatives: [],
    video_placeholder: { status: 'none', asset_id: null, note: 'Video placeholder — no asset linked yet.' },
    stop_rule: betaStopRule(),
    log_prompts: kind === 'warmup'
      ? ['Did you complete the warm-up?']
      : ['Did you complete the cooldown?', 'How does the leg feel now?'],
...
export function buildSession(eligiblePool, phaseId, sessionIndex = 0, _maxCards = 6) {
  ...
  // Group pool by block. Preserve the incoming pool order within each block —
  // the pool has already been ranked by the RAG retriever (most clinically
  // appropriate exercise first). Re-sorting by gentleness here would overwrite
  // that ranking and cause sport-mismatched exercises to be selected first.
  // Gentleness ordering is applied later when assembling the final card list
  // so the session still flows from easiest to hardest.
  ...
  const rfCards = picked.map(({ ex, blockSpec }) => {
    const card = toCard(ex, blockSpec, eligiblePool);
    const { _gentleness, ...rest } = card;
    return rest;
  });

  // Bookend every session with a phase-appropriate RAMP warm-up (first) and a
  // cooldown + recovery block (last) — the structural pieces a real rehab
  // session always has. Shape-safe: same card shape as every other card.
  const warmupCard   = makeBookendCard('warm_up',  getWarmupContent(phaseId),  phaseId, 'warmup');
  const cooldownCard = makeBookendCard('cool_down', getCooldownContent(phaseId), phaseId, 'cooldown');
  const cards = [warmupCard, ...rfCards, cooldownCard];
  ...
  return {
    ...
    physio_disclaimer: tierConfig.physio_disclaimer ?? false,
    blocks,
    cards,
  };
}
