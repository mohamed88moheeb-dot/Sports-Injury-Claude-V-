...
function devPhasePool(phaseId, equipment, isCurrent, retrievalProfile) {
  ...
  // RAG: rank by clinical relevance when a Bayesian pattern OR a deficit signal
  // is available (deficits alone are enough to personalise selection).
  if (retrievalProfile && (retrievalProfile.pattern || retrievalProfile.deficits_active)) {
...
export function generatePlan(severity, options = {}) {
  ...
  const retrievalProfile = buildPatientProfile(confidence, rawAnswers, input);
