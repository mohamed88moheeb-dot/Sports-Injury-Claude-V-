...
export function generatePlan(severity, options = {}) {
  ...
  const phases = PHASES.map((ph, idx) => {
    ...
    // Current phase: only auto-selectable exercises, properly prescribed.
    // All other phases: full preview pool (selectable + withheld) shown as roadmap.
    const isCurrent = status === 'current';
    const pool = isCurrent ? selectable : phasePreviewPool(ph.id);
    const days = buildCurrentWeek(ph, pool, band, isCurrent ? maxCards : (MAX_ACTIVE_CARDS_PER_PHASE[ph.id] || 4));
