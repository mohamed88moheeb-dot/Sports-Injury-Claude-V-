/**
 * lib/clinical/gluteEngine/appAdapter/gluteOutputToProfile.mjs
 * ---------------------------------------------------------------------------
 * Transforms glute engine output into the EXISTING RecoveryContext `profile`
 * shape. Mirrors hipFlexorOutputToProfile.mjs / groinOutputToProfile.mjs.
 * ---------------------------------------------------------------------------
 */
...
const ENTITY_TITLES = {
  gluteal_strain: 'Acute gluteal muscle strain',
  gluteal_tendinopathy: 'Gluteal tendinopathy / greater trochanteric pain syndrome',
  possible_deep_gluteal_syndrome: 'Possible deep gluteal syndrome (sciatic nerve) — needs assessment',
};
...
/**
 * @param {object} gluteOutput  runGlute() output
 * @param {object} appAssessment  the legacy assessment
 * @returns {object} profile WITHOUT progress/today (caller computes those)
 */
export function gluteOutputToProfile(gluteOutput, appAssessment = {}, extras = {}) {
  const entity = gluteOutput.entity || gluteOutput.routing?.entity || 'gluteal_strain';
  const diagnosis = gluteOutput.diagnosis || {};
  const gplan = gluteOutput.plan || null;

  const referral = gluteOutput.referral || null;
  const isReferral = gluteOutput.autonomous_plan === false && !gplan;

  const plan = stagesToProfilePlan(gplan?.stages, mapSession);

  const severity = diagnosis.severity || diagnosis.assessment || {};
  const timeline = describeRecoveryTimeline(gplan?.total_estimated_weeks, severity.prognosis);
  const reviewRequired = !!(gluteOutput.clinician_required || isReferral || diagnosis.flags?.includes('urgent_in_person_review'));
  ...
  const sportParticipation = gluteOutput.sport_participation || deriveEnvelopeFromSharedOutput(gluteOutput);

  const MATCH = { high: 80, moderate: 66, low: 52 };
  const routedScore = Number(gluteOutput.routing?.confidence_score);
  ...
  const matchScore = Number.isFinite(routedScore) && routedScore > 0
    ...
    : (MATCH[gluteOutput.routing?.confidence] ?? 60);
  const matchLabel = { high: 'Good match', moderate: 'Moderate match', low: 'Limited match' }[gluteOutput.routing?.confidence] || '';
  ...
  return {
    ...
    primaryRegion: appAssessment.primaryRegion || 'glutes',
    regionName: 'Glutes',
    injuryTitle: ENTITY_TITLES[entity] || 'Glute injury',
    exactAreaName: (appAssessment.exactArea ? pretty(appAssessment.exactArea).replace(/^\w/, (c) => c.toUpperCase()) : '') || ENTITY_TITLES[entity] || 'Glutes',
    gradeName,
    mechanism: describeMechanism(gluteOutput.input),
    ...
    daysSinceInjury: Number.isFinite(Number(gluteOutput.input?.days_since_injury)) ? Number(gluteOutput.input.days_since_injury) : null,
    diagnosisDrivers: diagnosisDrivers(gluteOutput.routing),
    ...
    rfReviewMessage: referral?.message || (gluteOutput.gate || null),
    ...
    gluteEntity: entity,
    gluteStretchPolicy: gluteOutput.stretch_policy || null,
    gluteReferral: referral || null,
    gluteDiagnosisSummary: diagnosis.summary || '',
    gluteConfidenceScore: gluteOutput.routing?.confidence_score ?? null,
    ...
    aiStatus: isReferral
      ...
      : 'Glute beta plan. Start gently, log daily, and use the check-in to adjust today only.',
    planNote: 'Glute beta plan — beta dosage defaults, not evidence-graded; requires clinician review before production use.',
  };
}
