...
function stageDurations(entity, band) {
  const totalDaysRaw = (TOTAL_RECOVERY_DAYS[entity] || TOTAL_RECOVERY_DAYS.hamstring_mtj_strain)[band] || 42;
  // Same largest-remainder week allocation the other engines use (via the
  // shared core), so a stage's weeks always sum EXACTLY to the displayed
  // total — no independent per-stage rounding that silently drifts the total.
  const weeksByStage = splitStageWeeks(STAGES, totalDaysRaw / 7, STAGE_SHARE);
  const effectiveTotalWeeks = Object.values(weeksByStage).reduce((a, b) => a + b, 0);
  const days = {};
  for (const s of STAGES) days[s.id] = weeksByStage[s.id] * 7;
  return { days, weeksByStage, totalDays: effectiveTotalWeeks * 7, totalWeeks: effectiveTotalWeeks };
}
...
export function buildHamstringPlan(diagnosis, input) {
  ...
  const { days: stageDays, weeksByStage, totalDays, totalWeeks } = stageDurations(diagnosis.entity, diagnosis.band);
  const { stageId: placedStageId, daysIntoStage, stageDurationDays } = locateInTimeline(diagnosis.entity, diagnosis.band, daysSinceInjury);
  const tier = estimateTier(daysIntoStage, stageDurationDays);

  // Minor presentations get 0-week early stages from splitStageWeeks — drop
  // them so a mild strain runs a genuinely short plan. If the clinically-
  // placed entry stage was trimmed, re-instate it in place of the first kept
  // phase (don't skip an acute presentation straight to late-stage work).
  let activeStages = STAGES.filter((s) => (weeksByStage[s.id] || 0) > 0);
  const effWeeksByStage = { ...weeksByStage };
  if (!activeStages.some((s) => s.id === placedStageId)) {
    const placed = STAGES.find((s) => s.id === placedStageId);
    if (placed && activeStages.length) {
      effWeeksByStage[placed.id] = effWeeksByStage[activeStages[0].id] || 1;
      activeStages = [placed, ...activeStages.slice(1)];
    }
  }
  const currentStageId = activeStages.some((s) => s.id === placedStageId) ? placedStageId : activeStages[0].id;
  const curIdx = activeStages.findIndex((s) => s.id === currentStageId);
  ...
  const stages = activeStages.map((stage, idx) => {
    ...
    const stageWeeks = effWeeksByStage[stage.id];

    // Materialize the REAL number of weeks for this stage (not just one),
    // continuing the rotating-pool's session numbering across weeks so a
    // multi-week stage doesn't repeat one identical week verbatim.
  ...
  return {
    ...
    current_stage_name: activeStages[curIdx]?.name || 'Protect & activate',
