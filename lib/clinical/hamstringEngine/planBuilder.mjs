 ...
 *     — e.g. Askling range progresses extender → diver → glider; Nordic reps
 *     climb 5 → 6 → 8; RDL load goes light → moderate → heavy — instead of a
 *     single flat session repeated for the whole stage.
 *
 * The rendered "week" is the CURRENT week only (mirrors RF's rfOutputToProfile,
 * which also exposes one representative week per phase); the tier reflects how
 * far into the stage the athlete is estimated to be.
 * ---------------------------------------------------------------------------
 */
...
// Total recovery estimate (days) per entity × band, split proportionally
// across the four stages. Mirrors RF's PHASE_DURATION_DAYS approach.
...
// Proportional share of total recovery time per stage (sums to 1).
...
// 'S' = training session, 'R' = rest/recovery day. Frequency is evidence-based:
// isometrics tolerate near-daily loading (protect); eccentric/lengthening work
// needs 48h+ recovery between sessions (lengthen/strengthen, Nordic literature);
// return-to-sport running load is capped to protect adaptation.
const WEEK_PATTERN = {
  protect: ['S', 'R', 'S', 'R', 'S', 'R', 'R'],      // ~3x/week structured + daily gentle activation implied in guidance
  lengthen: ['S', 'R', 'S', 'R', 'S', 'R', 'R'],     // 3x/week — Askling protocol pacing
  strengthen: ['S', 'R', 'S', 'R', 'S', 'R', 'R'],   // 3x/week — Nordic eccentric needs recovery between sessions
  return: ['S', 'R', 'S', 'R', 'S', 'R', 'R'],       // 3x/week high-speed running + sport days
};
const SESSIONS_PER_WEEK = 3;

/** 0 = early, 1 = mid, 2 = late tier within the current stage. */
...
/** Which stage + how many days into it, from days_since_injury and stage durations. */
...
// ── Tiered exercise banks (dosage escalates early -> mid -> late) ──────────
// Each entry is a function(tier) -> exercise, so dosage/range genuinely changes
// across the stage rather than repeating one flat prescription.
function strainBank(entity, sport) {
  ...
  return {
    ...
    lengthen: (tier) => {
      ...
      if (tier >= 1) cards.push(ex('Askling "diver" (single-leg hip hinge to horizontal)', 'tissue_specific_loading', 'Eccentric control through range with balance demand',
      ...
      if (tier >= 2) cards.push(ex('Askling "glider" (slide-board eccentric)', 'tissue_specific_loading', 'Progressive eccentric lengthening',
...
export function buildHamstringPlan(diagnosis, input) {
  ...
  const stages = STAGES.map((stage, idx) => {
    ...
    const coreExercises = bank[stage.id](stageTier);
    const stageWeeks = Math.max(1, Math.round(stageDays[stage.id] / 7));

    // Build ONE representative week for this stage: real session/rest pattern,
    // each session pulling the stage's tiered exercise set (not one flat copy).
    ...
    const sessions = pattern.map((dayType, dayIdx) => {
      ...
      sessionNum++;
      const cards = [WARMUP[stage.id], ...coreExercises, COOLDOWN[stage.id]].filter(Boolean);
      return {
    ...
    return {
      ...
      sessions,
      exercise_count: coreExercises.length,
    };
  });
