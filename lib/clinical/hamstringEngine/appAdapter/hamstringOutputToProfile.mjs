...
export function hamstringOutputToProfile(out, appAssessment = {}) {
  ...
  let dayCounter = 1;
  const plan = (out.plan?.stages || []).map((stage) => {
    ...
    return {
      ...
      sessions_per_week: stage.sessions_per_week || null,
  ...
  const rf = {
    ...
    recovery: { wording: 'Estimated window for this pattern — actual recovery depends on injury extent, symptoms, sport demands, and review.', beta_display: dx.recovery, modifiers: dx.recovery_drivers || [] },
  ...
  return {
    ...
    mechanism: MECHANISM_LABEL[out.input?.mechanism] || appAssessment.mechanism || '',

    returnRange: dx.recovery,
    rfRecoveryWording: rf.recovery.wording,
