...
// "Can I keep playing?" — tendinopathy is load-managed alongside sport;
// acute strains map band → grade (lower≈I, moderate≈II, high-concern≈III).
// Exported so the API route can use it as the AI grounding layer's safety
// envelope while this adapter keeps it as the client-side fallback.
export function deriveHamstringParticipationEnvelope(out = {}) {
  const dx = out.diagnosis || {};
  const reviewRequired = !!(out.review_required || out.autonomous_plan === false);
  const hamStages = out.plan?.stages || [];
  const bandToGrade = { lower: 'grade_1', moderate: 'grade_2', high_concern: 'grade_3' };
  const bandToIrr = { lower: 'low', moderate: 'moderate', high_concern: 'high' };
  const isTendinopathy = dx.entity === 'proximal_hamstring_tendinopathy';
  return deriveSportParticipation({
    kind: isTendinopathy ? 'overuse' : 'acute_strain',
    grade: isTendinopathy ? null : (bandToGrade[dx.band] || null),
    irritability: isTendinopathy ? (bandToIrr[dx.band] || 'moderate') : null,
    band: dx.band || null,
    reviewRequired,
    planTotalWeeks: out.plan?.total_estimated_weeks || null,
    finalStageWeeks: hamStages.length ? (hamStages[hamStages.length - 1].duration_weeks || null) : null,
  });
}
...
export function hamstringOutputToProfile(out, appAssessment = {}) {
  ...
  const sportParticipation = out.sport_participation || deriveHamstringParticipationEnvelope(out);
