...
export function mapRfAnswersToRfInput(answers = {}, base = {}) {
  // Translate new resisted-test option values → engine enums.
  // pain_and_weakness → 'painful' (not 'unable') so the severe gate only triggers
  // for true inability, not pain-with-weakness. Weakness signal is captured separately
  // via weakness_or_giving_way. weakness_no_pain → 'weak' (neural concern flag).
  function toEngineEnum(val) {
    const map = {
      no_pain:           'no',
      mild_pain_strong:  'mild',
      significant_pain:  'painful',
      pain_and_weakness: 'painful',  // pain + weakness ≠ unable; don't trigger severe gate
      weakness_no_pain:  'weak',
      not_tried:         'unsure',
    };
  ...
  // weakness_or_giving_way: 'marked' if either resisted test showed weakness or inability.
  // Includes both 'weak' (weakness_no_pain) and the raw pain_and_weakness option.
  const weak =
    rkeEngine === 'weak' || rkeEngine === 'unable' ||
    rhfEngine === 'weak' || rhfEngine === 'unable' ||
    answers.knee_extension_response === 'pain_and_weakness' ||
    answers.resisted_hip_flexion === 'pain_and_weakness';
  ...
  return {
    ...
    rf_self_tests: {
      ...
      bruising_timing:           answers.bruising_timing           || null,
      age_group:                 answers.age_group                 || null,
      pain_at_rest:              answers.pain_at_rest              || null,
      previous_injury_detail:    answers.previous_injury_detail    || null,
      // Raw (untranslated) resisted-test values — engine uses these for graded LRs
      // so each answer option contributes its own weight (not lumped into binary).
      knee_extension_raw:        answers.knee_extension_response   || null,
      resisted_hip_flex_raw:     answers.resisted_hip_flexion      || null,
      sport_level:               answers.sport_level               || null,
    },
    days_since_injury,
