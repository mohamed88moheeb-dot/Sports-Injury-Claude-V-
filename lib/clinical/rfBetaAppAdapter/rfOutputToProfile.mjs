...
import { describeMechanism } from '../core/diagnosisDisplay.mjs';
...
export function rfOutputToProfile(rfOutput, appAssessment = {}) {
  ...
  return {
    ...
    // Mechanism from the RF assessment's own answers — never the untouched
    // generic-assessment default, which the RF flow doesn't even display.
    mechanism: describeMechanism(rfOutput.input),
    ...
    // RF is deliberately capacity/criteria-gated: no promised week total, so
    // the diagnosis page hides the time axis rather than faking one.
    planTotalWeeks: null,
    daysSinceInjury: null,
    diagnosisDrivers: severity.severity_reasons || [],
    diagnosisNote: null,
    // RF strains: grade inferred from the governed severity wording; falls
    // back to the cautious grade-II guidance when the wording is ambiguous.
    sportParticipation: deriveSportParticipation({
      kind: 'acute_strain',
      grade: /mild|grade\s*i\b|low/i.test(severity.severity_grade_wording || '') ? 'grade_1'
        : /severe|grade\s*iii|rupture/i.test(severity.severity_grade_wording || '') ? 'grade_3'
          : 'grade_2',
      reviewRequired,
    }),
