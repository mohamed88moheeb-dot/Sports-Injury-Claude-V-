...
const ENTITY_TITLES = {
  ...
  possible_inguinal_hernia: 'Possible inguinal hernia — needs assessment',
};
...
export function groinOutputToProfile(groinOutput, appAssessment = {}, extras = {}) {
  ...
  const sportParticipation = groinOutput.sport_participation || deriveEnvelopeFromSharedOutput(groinOutput);

  const MATCH = { high: 80, moderate: 66, low: 52 };
  const routedScore = Number(groinOutput.routing?.confidence_score);
  ...
  const matchScore = Number.isFinite(routedScore) && routedScore > 0
    ...
    : (MATCH[groinOutput.routing?.confidence] ?? 60);
  ...
  return {
    ...
    exactAreaName: (appAssessment.exactArea ? pretty(appAssessment.exactArea).replace(/^\w/, (c) => c.toUpperCase()) : '') || ENTITY_TITLES[entity] || 'Adductor/groin',
    gradeName,
    mechanism: describeMechanism(groinOutput.input),
    ...
    daysSinceInjury: Number.isFinite(Number(groinOutput.input?.days_since_injury)) ? Number(groinOutput.input.days_since_injury) : null,
    diagnosisDrivers: diagnosisDrivers(groinOutput.routing),
